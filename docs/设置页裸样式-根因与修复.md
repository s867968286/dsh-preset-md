# 设置页「裸 HTML」根因与修复

> 现象：设置 → 伙伴设置 整页没有任何样式（像裸 HTML）。
> 结论：**client 半引用了已不存在的 npm 包，factory 抛错，`apply` 从未执行。**
> 排查日期：2026-09-13

---

## 一、根因

`client/client.js` 第 13 行（`factory` 顶层）：

```js
const { Modal, Button, IconBrowseOutline16, IconCopyOutline16, IconTrashOutline16 } = require('@deepseek-ai/dsh-client-ui-primitives')
```

**这个包在当前 DSH 版本里已经不存在了。** 实测：

```
$ node -e "require.resolve('@deepseek-ai/dsh-client-ui-primitives')"
FAIL  MODULE_NOT_FOUND

$ node -e "require.resolve('@deepseek-ai/dsh-client-ui-settings')"
OK
```

```
$ find <dsh安装目录> -name "*primitives*"
（无结果）
$ grep ui-primitives <dsh>/package.json
（未提及）
```

### 为什么会整页裸样式

链条很短：

1. `factory` 在**顶层**解构 → 模块不存在 → **factory 抛错**
2. `factory` 抛错 → 返回的 plugin 对象不存在 → **`apply(ctx)` 从不执行**
3. `apply` 里的 `ensureStyles()` 从不调用 → **`<style>` 从不插入 DOM**
4. 于是整页只有类名、没有规则 → 裸 HTML

注意：**不是样式写错了，是样式代码根本没跑到。** 所以调 CSS 是白费力气。

### 为什么以前没发现

它一直是坏的，只是之前机器上只有这一个设置页，没有对照就看不出来。
装上第二个自研设置页（`dsh-memory-md`）后两个并排，一眼就露馅了。

---

## 二、修复方式

**思路：不依赖 `primitives`，用 `React.createElement` + 原生元素自己实现。**

`primitives` 提供的四类东西，替代方案都很直接：

| 原来 | 替代 |
|---|---|
| `Modal` | 自建 overlay + dialog（`position:fixed` 遮罩 + 内容卡） |
| `Button` | 原生 `<button>` + 现有 `.pmd-btn` 类 |
| `IconBrowseOutline16` / `IconCopyOutline16` / `IconTrashOutline16` | 内联 SVG |

### 1. 删掉第 13 行

```diff
- const { Modal, Button, IconBrowseOutline16, IconCopyOutline16, IconTrashOutline16 } = require('@deepseek-ai/dsh-client-ui-primitives')
```

改完这一行，**整页裸样式的问题立刻消失**（因为 factory 不再抛错）。
下面的 2、3 是为了让两个弹窗和三个图标按钮也恢复功能。

### 2. 两个 `Modal` 调用点改成自建弹窗

涉及 **第 310 行**（复制伙伴）与 **第 428 行**（日记查看器）。

在 `client/client.js` 里加一个组件：

```js
/**
 * 自建弹窗。
 * 不用 @deepseek-ai/dsh-client-ui-primitives —— 该包在此 DSH 版本已不存在。
 * 属性含义与原 Modal 对齐：title / description / footer / onClose / className。
 */
function Dialog({ open, onClose, title, description, footer, className, children }) {
  if (!open) return null
  return h('div', {
    className: 'pmd-overlay',
    role: 'dialog',
    'aria-modal': 'true',
    'aria-label': title,
    onClick: onClose,                       // 点遮罩关闭
  },
    h('div', {
      className: `pmd-dialog-sheet ${className || ''}`.trim(),
      onClick: (event) => event.stopPropagation(),   // 点内容不关闭
    },
      h('div', { className: 'pmd-dialog-head' },
        h('div', { className: 'pmd-dialog-title' }, title),
        description ? h('div', { className: 'pmd-dialog-sub' }, description) : null,
      ),
      children,
      footer ? h('div', { className: 'pmd-dialog-foot' }, footer) : null,
    ),
  )
}
```

**第 310 行**处：

```diff
-      const copyDialog = copyFrom ? h(Modal, {
-        open: true,
-        onClose: closeCopy,
+      const copyDialog = copyFrom ? h(Dialog, {
+        open: true,
+        onClose: closeCopy,
```

其余 props（`title` / `closeLabel` / `className` / `description` / `footer` / children）**保持不变**，
`Dialog` 的签名与原 `Modal` 对齐，所以只需改组件名。`closeLabel` 现在没用到，留着无害。

同样把两个 `h(Button, { variant: 'outline', ... }, '关闭')` 换成原生按钮：

```diff
-          h(Button, { variant: 'outline', disabled: copying, onClick: closeCopy }, '取消'),
-          h(Button, { disabled: copying || !copyName.trim(), onClick: confirmCopy }, copying ? '复制中…' : '复制')),
+          h('button', { type: 'button', className: 'pmd-btn', disabled: copying, onClick: closeCopy }, '取消'),
+          h('button', { type: 'button', className: 'pmd-btn pmd-btn-primary', disabled: copying || !copyName.trim(), onClick: confirmCopy }, copying ? '复制中…' : '复制')),
```

**第 428 行**处同理：

```diff
-      const viewer = h(Modal, {
-        open: viewing !== null,
+      const viewer = h(Dialog, {
+        open: viewing !== null,
         onClose: () => setViewing(null),
         title: viewing ? `日记 · ${viewing}` : '',
         closeLabel: '关闭',
         className: 'pmd-viewer-dialog',
         description: '这一天的日记内容，只读。',
-        footer: h(Button, { variant: 'outline', autoFocus: true, onClick: () => setViewing(null) }, '关闭'),
+        footer: h('button', { type: 'button', className: 'pmd-btn', autoFocus: true, onClick: () => setViewing(null) }, '关闭'),
       }, h('pre', { className: 'pmd-viewer' }, loading ? '读取中…' : text))
```

### 3. 三个图标改成内联 SVG

涉及 **第 360 / 367 / 375 行**。都是 `h(IconXxxOutline16)`，替换为等尺寸（16×16）内联 SVG：

```js
const ICON_PROPS = {
  width: 16, height: 16, viewBox: '0 0 16 16', fill: 'none',
  stroke: 'currentColor', strokeWidth: 1.5,
  strokeLinecap: 'round', strokeLinejoin: 'round',
}
const IconBrowse = () => h('svg', ICON_PROPS,
  h('circle', { cx: 7, cy: 7, r: 4.5 }), h('path', { d: 'M10.5 10.5 L14 14' }))
const IconCopy = () => h('svg', ICON_PROPS,
  h('rect', { x: 5.5, y: 5.5, width: 8, height: 8, rx: 1.5 }),
  h('path', { d: 'M10.5 5.5 V3.5 A1.5 1.5 0 0 0 9 2 H3.5 A1.5 1.5 0 0 0 2 3.5 V9 A1.5 1.5 0 0 0 3.5 10.5 H5.5' }))
const IconTrash = () => h('svg', ICON_PROPS,
  h('path', { d: 'M2.5 4.5 H13.5' }), h('path', { d: 'M6 4.5 V3 A1 1 0 0 1 7 2 H9 A1 1 0 0 1 10 3 V4.5' }),
  h('path', { d: 'M4 4.5 L4.8 13 A1 1 0 0 0 5.8 14 H10.2 A1 1 0 0 0 11.2 13 L12 4.5' }))
```

用于按钮时保持 `stroke: currentColor`，就能跟随 `.pmd-icon-btn` / `.pmd-icon-btn-danger`
的文字色 —— hover 变红等现有行为不受影响。

然后把三处 `h(IconXxxOutline16)` 改成 `h(IconXxx)`。

### 4. 补 CSS

`Modal` 原来带自己的定位与层级，自建的要补上（加进 `CSS` 数组）：

```js
'.pmd-overlay{position:fixed;inset:0;z-index:1000;display:flex;align-items:center;justify-content:center;padding:24px;background:var(--dsw-alias-bg-mask-drop,rgba(0,0,0,.45))}',
'.pmd-dialog-sheet{width:min(560px,100%);max-height:min(80vh,720px);box-sizing:border-box;display:flex;flex-direction:column;gap:10px;padding:16px 18px;border-radius:14px;border:.5px solid var(--dsw-alias-border-l2,#e5e5e5);background:var(--dsw-alias-bg-overlay,#fff);color:var(--dsw-alias-label-primary,#1f1f1f);box-shadow:0 12px 40px rgba(0,0,0,.18)}',
'.pmd-dialog-head{display:flex;flex-direction:column;gap:2px}',
'.pmd-dialog-title{font-size:15px;font-weight:600;line-height:1.4;overflow-wrap:anywhere}',
'.pmd-dialog-sub{font-size:11.5px;color:var(--dsw-alias-label-tertiary,#8c8c8c)}',
'.pmd-dialog-foot{display:flex;justify-content:flex-end;gap:8px}',
```

`className` 传进来的 `pmd-dialog` / `pmd-viewer-dialog` 仍会拼在 `pmd-dialog-sheet` 上，
原有宽度覆盖（`.pmd-viewer-dialog{width:min(640px,100%)}`）继续生效。

---

## 三、验证

改完**不需要重装**，但 client 半由前端模块系统加载，**刷新页面**即可（Host 半没动，不用重启 dsh）。

1. 刷新页面 → 设置 → 伙伴设置 → 样式应该回来了
2. 点卡片底部三个图标按钮 → 查看 / 复制 / 删除 都应正常
3. 打开「复制伙伴」弹窗、点开某天日记 → 弹窗应正常显示与关闭

如果**仍然裸样式**，说明还有别的顶层 `require` 失败 —— 按 F12 看 Console
里的第一个报错，那就是下一个要处理的模块。

---

## 三补、后续修正（图标已改回官方）

> 修正日期：本节的结论**推翻了上文第二节第 3 条的处置方式**（内联 SVG 是当时的临时替代，不是终态）。

上文判定「`@deepseek-ai/dsh-client-ui-primitives` 在当前 DSH 版本里已经不存在」，依据是
`require.resolve` 返回 `MODULE_NOT_FOUND`。**这个推断方法不成立**：

- 官方 `dsh-client-ui-agent-preset/lib/client.js:9` 在顶层 `require` 同一个包，
  并在 `:1118 / :1138 / :1148` 使用 `IconBrowseOutline16 / IconCopyOutline16 /
  IconTrashOutline16` —— 正是卡片底部那三个按钮；
- 该包 `package.json` 的 `dependencies` 为空 `{}`。

说明它由**前端模块系统在运行时注入**，不走 Node 的磁盘解析。
**Node 里 `require.resolve` 失败 ≠ 浏览器里 require 失败。**

因此现在改成运行时探测 + 回退（`client/client.js` 的 `pickOfficialIcon`）：

- 优先取官方同名图标，与官方预设卡片视觉一致；
- `require` 包在 `try/catch` 内，取不到就退回内联 SVG。

**关键约束：这个 `require` 绝不能挪到 `try/catch` 之外。** 上文第一节的因果链仍然成立 ——
顶层抛错会让 `apply` 从不执行、整页裸样式。测试已按此锁定：
「client 对官方包的 require 必须被 try/catch 包住」。

---

## 四、教训

**client 半的任何外部 `require`，都必须先确认包真的存在，不能靠「别的插件这么写」推断。**
并且**判断「包在不在」要用浏览器侧的运行时探测，不能用 Node 的 `require.resolve`** ——
后者对前端模块系统注入的包会给出假阴性（见上文「三补」）。
若确实需要引用官方包，放进 `try/catch` + 回退，永远不要放在 `factory` 顶层裸解构。

一行验证就够（仅适用于真正走 Node 解析的包）：

```bash
node -e "require.resolve('<包名>')"
```

本项目的 `client/client.js` 只应依赖 `react`（由前端模块系统提供）。
如需再从官方包引 UI 组件，请先验证；官方组件的包名在 DSH 版本间会变。
